...
# ====================== НАСТРОЙКИ ======================
INPUT_FILE = "power_data_correlated_with_anomalies_s2.xlsx"   # ← поменяй при необходимости
OUTPUT_FILE = "power_data_OCSVM_8features_predictions.xlsx"
MODEL_PATH = "models/ocsvm_8features_model.pkl"               # путь к сохранённой модели

ANOMALY_START_DAY = 61
START_DATETIME = datetime(2025, 10, 5, 0, 2)

print("=== One-Class SVM (8 features) — Применение сохранённой модели ===\n")
...
print("Модель One-Class SVM (8 features) успешно загружена")
print(f"Параметры: nu={model_info['NU']}, kernel={model_info['KERNEL']}, gamma={model_info['GAMMA']}\n")
...
# ====================== ПРИМЕНЕНИЕ МОДЕЛИ ======================
print("Применяем One-Class SVM к данным...")

df['OCSVM_Label'] = ocsvm.predict(X_full_scaled)
df['OCSVM_score'] = ocsvm.score_samples(X_full_scaled)
df['OCSVM_decision'] = ocsvm.decision_function(X_full_scaled)

df['Is_OCSVM_Anomaly'] = (df['OCSVM_Label'] == -1).astype(int)
df['anomaly_score'] = -df['OCSVM_decision']   # чем меньше — тем аномальнее

# ====================== МЕТРИКИ (если есть ground truth) ======================
if 'Is_Anomaly' in df.columns:
    df['y_true'] = df['Is_Anomaly'].map({'Да': 1, 'Нет': 0}).fillna(0).astype(int)
    df['y_pred'] = df['Is_OCSVM_Anomaly']
...
total_anom = df['Is_OCSVM_Anomaly'].sum()
period_anom = df.loc[df['day_num'] >= ANOMALY_START_DAY, 'Is_OCSVM_Anomaly'].sum()
