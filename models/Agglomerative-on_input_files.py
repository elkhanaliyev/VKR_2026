...
# ====================== НАСТРОЙКИ ======================
INPUT_FILE = "power_data_correlated_with_anomalies.xlsx"      # ← измени на нужный файл
OUTPUT_FILE = "power_data_Agglomerative_4features_predictions.xlsx"
MODEL_PATH = "models/agglomerative_4features_model.pkl"

ANOMALY_START_DAY = 61
START_DATETIME = datetime(2025, 10, 5, 0, 2)

print("=== Agglomerative Clustering (4 features) — Inference ===\n")
...
agg = model_info['model']
scaler = model_info['scaler']
centroids = model_info['centroids']
distance_threshold = model_info['distance_threshold']
features = model_info['features']

print("Модель Agglomerative Clustering (4 features) успешно загружена")
print(f"Порог расстояния: {distance_threshold:.4f}\n")
...
# ====================== ПРИМЕНЕНИЕ МОДЕЛИ ======================
print("Вычисляем расстояния до центроидов...")

X_full = df[features].values
X_full_scaled = scaler.transform(X_full)

# Минимальное расстояние до ближайшего центроида
full_dist = cdist(X_full_scaled, centroids, metric="euclidean").min(axis=1)

df['Agglo_min_dist'] = full_dist
df['Is_Agglo_Anomaly'] = (full_dist > distance_threshold).astype(int)
df['anomaly_score'] = full_dist                     # расстояние как anomaly score

# ====================== МЕТРИКИ (если есть ground truth) ======================
if 'Is_Anomaly' in df.columns:
    df['y_true'] = df['Is_Anomaly'].map({'Да': 1, 'Нет': 0}).fillna(0).astype(int)
    df['y_pred'] = df['Is_Agglo_Anomaly']
...
total_anom = df['Is_Agglo_Anomaly'].sum()
period_anom = df.loc[df['day_num'] >= ANOMALY_START_DAY, 'Is_Agglo_Anomaly'].sum()
